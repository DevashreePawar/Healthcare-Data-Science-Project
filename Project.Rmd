...
# -------------------------------
# 2. How many patients have repeated hospitalizations? (critical for all RQs)
# -------------------------------
long_df %>%
  summarise(Patients = n_distinct(patient_nbr),
            Encounters = n(),
            `Patients ≥2 visits` = sum(n() >= 2, na.rm = T, .by = patient_nbr),
            `% with repeated visits` = round(mean(n() >= 2, na.rm = T, .by = patient_nbr)*100, 1)) %>%
  knitr::kable(caption = "Longitudinal Structure of the Cohort")
...
# --- Define events cleanly (FIXED: use dplyr::lag) ---
long_df <- long_df %>%
  mutate(
    ...
    # First complication only — explicitly use dplyr::lag
...
p <- ggplot(plot_df, aes(x = time, y = est, color = event)) +
  geom_step(size = 1.8) +
...
ggsave(
  filename = "cumulative_incidence_competing_risks.png",
  plot = p,
  width = 10,
  height = 6,
  dpi = 300
...
```{r rq3-shap-fixed, fig.width=11, fig.height=7}
# ── FIXED SHAP EXPLANATION FOR XGBoost (no more "new_observation" error) ──
library(DALEXtra)
library(tidymodels)
library(dplyr)
library(ggplot2)
...
# Step 1: Fit final model on full data (uses your existing workflow)
final_xgb <- wf %>% 
  add_model(xgb_model_fast) %>% 
  fit(pred_df)
...
explainer_xgb <- explain_tidymodels(
  final_xgb,
  data = pred_df %>% select(-target),
  y = as.numeric(pred_df$target) - 1,  # 0/1 for XGBoost
  label = "XGBoost – Best Model"
)

# Step 3: Global SHAP importance (no new_observation needed)
shap_importance <- model_parts(explainer_xgb, type = "variable_importance", B = 25)

# Step 4: Individual SHAP summary (FIX: add new_observation = first 50 rows)
shap_summary <- predict_parts(
  explainer_xgb, 
  type = "shap", 
  new_observation = pred_df[1:50, ],  # Sample 50 patients for summary
  B = 25
)

# Step 5: Beautiful side-by-side plots
p1 <- plot(shap_importance) + 
  ggtitle("Global SHAP Feature Importance\n(Top Predictors of Readmission Risk)") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"))

p2 <- plot(shap_summary) + 
  ggtitle("SHAP Summary Plot\n(Individual Feature Contributions for 50 Patients)") +
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"))

# Display plots
p1 / p2  # Stacked for better space
```
