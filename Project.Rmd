...
**Goal**: Predict first major diabetic complication at the **next** hospital encounter using current + past data.

**Models**: 1. Logistic Regression 2. Random Forest 3. XGBoost

**Metrics**: ROC-AUC, PR-AUC, Brier Score on temporal test set
...
cat("Event rate in test set:", scales::percent(mean(test_data$target == "1")), "\n")
...
wf_xgb <- workflow() %>%
  add_recipe(rec) %>%
  add_model(boost_tree(trees = 400, learn_rate = 0.05) %>% set_engine("xgboost") %>% set_mode("classification"))
...
truth_factor <- test_data$target  # Factor with two levels
truth_num <- as.numeric(truth_factor) - 1  # Numeric 0/1

# --- Manual metrics (with factor for ROC) ---
calc_metrics <- function(prob, truth_f, truth_n) {
  c(
    ROC_AUC = as.numeric(auc(roc(truth_f, prob))),
    PR_AUC  = as.numeric(pr_auc_vec(truth_n, prob)),
    Brier   = mean((prob - truth_n)^2)
  )
}

results <- bind_rows(
  calc_metrics(p_log, truth_factor, truth_num) %>% t() %>% as.data.frame() %>% mutate(Model = "Logistic Regression"),
  calc_metrics(p_rf, truth_factor, truth_num) %>% t() %>% as.data.frame() %>% mutate(Model = "Random Forest"),
  calc_metrics(p_xgb, truth_factor, truth_num) %>% t() %>% as.data.frame() %>% mutate(Model = "XGBoost")
) %>%
  select(Model, ROC_AUC, PR_AUC, Brier) %>%
  arrange(desc(ROC_AUC))

knitr::kable(results, digits = 4, caption = "Final Model Performance Comparison")
```
